...
def build_distance_table(graph, source):
    # a distance mapping from vertex number to a tuple of
    ...
    # the distance from the initial from itself is 0
    ...
    while not queue.empty():
        ...
        for neighbor in graph.get_adjacent_vertices(current_vertex):
            # Only update the distance table if no current distancefrom
            # the source is set
